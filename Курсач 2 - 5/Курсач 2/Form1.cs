...
namespace Курсач_2
{
    public partial class Form1 : Form
    {
        ...
        public Variants[] vars = new Variants[0];
        ...
        public void ReturnToText(RichTextBox RTB, Variants[] v, int id)
        {
            int i;
            string path;
            for (i = 0; i < v.Length; i++)
            {
                if (v[i].id == id)
                {
                    path = @"D:\\DICT\" + v[i].variant[0] + @"\" + Convert.ToString(v[i].variant.Length + @".dat");
                    if (IsInDict(v[i].variant, path))
                    {
                        InsertWord(RTB, id, v[i].variant);
                    }
                }
            }
        }
