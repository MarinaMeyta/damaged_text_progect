            ...
            while (sub != "")
            {
                ...
                for (i = 0; i < sub.Length; i++)
                {
                    ...
                    if ((i == 0) && (sub[i] == '*'))                            //Если * в самом начале строки
                    {
                        ...
                        Array.Resize<string>(ref variants, 3);
                        for (k = 0; k < 3; k++)
                        {
                            variants[k] = sub;
                            variants[k] = variants[k].Remove(i, 1);
                            variants[k] = variants[k].Insert(i, Convert.ToString(Convert.ToChar(m1[k] + 1072)));
                            comboBox1.Items.Add(variants[k]);
                        }
                    }
                    if ((i == sub.Length - 1) && (sub[i] == '*'))               //Если * в конце строки
                    {
                        ...
                        for (k = 0; k < 3; k++)
                        {
                            variants[k] = sub;
                            variants[k] = variants[k].Remove(i, 1);
                            variants[k] = variants[k].Insert(i, Convert.ToString(Convert.ToChar(m0[k] + 1072)));
                            comboBox1.Items.Add(variants[k]);
                        }
                    }
                    if ((i > 0) && (i < sub.Length - 1) && (sub[i] == '*'))     //Если * где-то вдали от краев
                    {
                        ...
                        for (k = 0; k < 3; k++)
                        {
                            for (l = 0; l < 3; l++)
                            {
                                if (m0[k] == m1[l])
                                {
                                    if (variants.Length < count + 1)
                                        Array.Resize(ref variants, variants.Length + 1);
                                    variants[count] = sub;
                                    variants[count] = variants[count].Remove(i, 1);
                                    variants[count] = variants[count].Insert(i, Convert.ToString(Convert.ToChar(m0[k] + 1072)));
                                    comboBox1.Items.Add(variants[count]);
                                    count++;
                                }
                            }
                        }
                        label4.Text = variants[0];
                    }
                }
            }
        }
